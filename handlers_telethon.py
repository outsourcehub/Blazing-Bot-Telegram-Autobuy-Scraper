...
# Track failed verification attempts for second-click fallback
_failed_verification = set()  # Set of user_ids who failed verification on first click
...
async def help_handler(event):
    ...
    help_text = (
        ...
        "3. **Add a Source:** Add a private/public channel or group to monitor\n\n"
        "**How it works:**\n"
        ...
        "• If toggled on, sends valid contracts to your Blazing bot\n"
        "• Your Blazing bot executes the trade based on your settings\n\n"
        "**Need more help?**\n"
        "Contact support! theweb3scout@gmail.com | @CopyTradersHub"
    )
...
async def start_handler(event):
    ...
    if existing_user:
        ...
        if 'enabled_chains' not in user.settings:
            user.settings['enabled_chains'] = ['solana', 'bsc', 'ethereum', 'base', 'arbitrum', 'story', 'monad', 'sonic', 'avax', 'bera']
    ...
    # Check if user is verified (membership required)
    ...
    welcome_text = (
        ...
        "**Supported Chains:**\n"
        "• BSC (BNB) • Solana (SOL) • Ethereum (ETH) • Base (BASE) • Arbitrum (ARB)\n\n"
...
async def settings_handler(event):
    ...
    enabled_chains = user.settings.get('enabled_chains', ['solana', 'bsc', 'ethereum', 'base', 'arbitrum'])
    ...
    settings_text = (
        ...
        "Only contracts from enabled chains will be processed and sent to Blazing bot."
    )
...
async def callback_handler(callback_query):
    ...
    try:
        ...
        chain = 'solana'
        if params and params[-1] in ['solana', 'bsc', 'ethereum', 'base', 'arbitrum', 'story', 'monad', 'sonic', 'avax', 'bera']:
            chain = params[-1]
        ...
        # Only allow recheck_membership for unverified users
        if not user.is_verified and action not in ['recheck_membership']:
            await callback_query.answer(
                "❌ Access Denied\n\n"
                "You need to join the channel first and verify your membership.",
                alert=True
            )
            return
...
async def handle_menu_navigation(callback_query, menu_name: str, user, chain: str = 'solana'):
    ...
    if ':' in menu_name:
        ...
        menu_name = parts[0]
        if len(parts) > 1 and parts[1] in ['solana', 'bsc', 'ethereum', 'base', 'arbitrum', 'story', 'monad', 'sonic', 'avax', 'bera']:
            chain = parts[1]
    ...
    if menu_name == "main":
        ...
        enabled_chains = user.settings.get('enabled_chains', ['solana', 'bsc', 'ethereum', 'base', 'arbitrum', 'story', 'monad', 'sonic', 'avax', 'bera'])
        enabled_names = ', '.join([c.upper() for c in enabled_chains])
...
async def handle_setting_selection(callback_query, setting_name: str, user, chain: str = 'solana'):
    ...
    current_value = user.get_setting(setting_name)
...
async def handle_recheck_membership_callback(callback_query):
    """Two-click verification: first click checks, second click grants access"""
    global _failed_verification
    user_id = callback_query.sender_id
    user = storage.get_user(user_id)
    ...
    # Check if this is a second click from someone who already failed
    if user_id in _failed_verification:
        # Second click - grant access without checking
        _failed_verification.discard(user_id)
        
        current_time = time.time()
        storage.create_user(user_id=user_id, is_verified=True, verified_at=current_time)
        
        await callback_query.answer("✅ Access Granted!", alert=False)
        ...
        logger.info(f"✅ User {user_id} granted access via second click (no membership check)")
        return
    
    # First click - check membership
    is_member, message = await check_required_membership(user_id)
    
    if is_member:
        # User is member - grant permanent access
        current_time = time.time()
        storage.create_user(user_id=user_id, is_verified=True, verified_at=current_time)
        
        await callback_query.answer("✅ Welcome to Blazing Bot!", alert=False)
        ...
        logger.info(f"✅ User {user_id} verified - permanent access granted")
    else:
        # Not a member on first click - mark for second-click fallback
        _failed_verification.add(user_id)
        
        await callback_query.answer("Please join the channel first", alert=True)
        
        await callback_query.edit(
            "🚫 **Access Denied - Membership Required**\n\n"
            "You must join this channel to use this bot:\n\n"
            "• [WeCopyTradess](https://t.me/WeCopyTradess)\n\n"
            "After joining, click the button below again to verify:",
            buttons=[
                [Button.inline("✅ I've Joined - Verify Now", b"recheck_membership")]
            ]
        )
        logger.info(f"⏱️ User {user_id} first click failed - can click again for instant access")
...
async def send_contract_to_dbot(chain: str, address: str, user_id: int):
    """Send validated contract to Blazing trading bot"""
    logger.info(f"📤 SENDING CONTRACT TO BLAZING BOT")
    logger.info(f"   ├─ Chain: {chain.upper()}")
    logger.info(f"   ├─ Address: {address}")
    logger.info(f"   └─ User ID: {user_id}")

    # Single Blazing bot for all chains
    target_bot = '@blazing_trading_bot'
