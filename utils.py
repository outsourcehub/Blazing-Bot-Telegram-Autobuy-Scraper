...
LINK_PATTERNS = {
    ...
    'dexscreener': re.compile(r'dexscreener\.com/(solana|base|arbitrum|bsc|tron|ethereum|story|monad|sonic|avax|bera)/([A-Za-z0-9]{32,})(?:\?|$|\s)', re.IGNORECASE),
...
def format_settings_summary(user_settings: Dict[str, Any]) -> str:
    ...
    summary += f"💵 Amount/Percent: {format_setting_display('amountOrPercent', user_settings.get('amountOrPercent'))}\n"
    summary += f"📊 Max Slippage: {format_setting_display('maxSlippage', user_settings.get('maxSlippage'))}\n"
    summary += f"🔄 Retries: {user_settings.get('retries', 'Not set')}\n"
    summary += f"⚡ Concurrent Nodes: {user_settings.get('concurrentNodes', 'Not set')}\n\n"

    # Buy Settings - Gas & Fees
    summary += "**⛽ Gas & Fees**\n"
    summary += f"🛡️ Jito Enabled: {format_setting_display('jitoEnabled', user_settings.get('jitoEnabled'))}\n"
    summary += f"💸 Jito Tip: {format_setting_display('jitoTip', user_settings.get('jitoTip'))}\n"
    summary += f"🔧 Custom Fee & Tip: {format_setting_display('customFeeAndTip', user_settings.get('customFeeAndTip'))}\n"
    summary += f"⚡ Priority Fee: {format_setting_display('priorityFee', user_settings.get('priorityFee'))}\n\n"

    # Sell Settings
    summary += "**📤 SELL SETTINGS**\n"
    summary += f"🚀 Migrate Sell %: {format_setting_display('migrateSellPercent', user_settings.get('migrateSellPercent'))}\n"
    summary += f"👨‍💻 Min Dev Sell %: {format_setting_display('minDevSellPercent', user_settings.get('minDevSellPercent'))}\n"
    summary += f"📊 Dev Sell %: {format_setting_display('devSellPercent', user_settings.get('devSellPercent'))}\n\n"

    # Active Chains
    summary += "**🌐 ACTIVE CHAINS**\n"
    enabled_chains = user_settings.get('enabled_chains', ['solana'])
    summary += f"Enabled: {', '.join([c.upper() for c in enabled_chains])}\n"
